...
export const api = {
  ...
  // One-time pull from a public Google Sheet URL, right from the Contacts
  // page — distinct from googleSheetSettings (the persistent auto-sync
  // configured in Admin Settings). Returns the same {imported,updated,skipped}
  // shape as importContacts so both can share one result-summary UI.
  importContactsFromSheet: (waNumber, sheetUrl) =>
    req('/contacts/import-sheet', { method: 'POST', body: JSON.stringify({ waNumber, sheetUrl }) }),
  categories: {
    list: () => req('/categories'),
  ...
  targetMessage: {
    customers: ({ search = '', filters = [], combinator = 'AND', limit = 500 } = {}) => {
      const qs = new URLSearchParams({ search, combinator, limit: String(limit) });
      if (filters.length > 0) qs.set('filters', JSON.stringify(filters));
      return req(`/target/customers?${qs}`);
    },
    createCampaign: (data) => req('/target/campaign', { method: 'POST', body: JSON.stringify(data) }),
  },
  googleSheetSettings: {
    get: () => req('/google-sheet-settings'),
    save: (sheetUrl, sheetName) =>
      req('/google-sheet-settings', { method: 'POST', body: JSON.stringify({ sheetUrl, sheetName }) }),
    syncNow: () => req('/google-sheet-settings/sync-now', { method: 'POST' }),
    syncLog: () => req('/google-sheet-settings/sync-log'),
  },
  broadcasts: {
  ...
  instagram: {
    inbox: {
      list: () => req('/instagram/inbox'),
      messages: (conversationId) => req(`/instagram/inbox/${conversationId}/messages`),
    },
    contacts: {
      list: () => req('/instagram/contacts'),
      save: (data) => req('/instagram/contacts', { method: 'POST', body: JSON.stringify(data) }),
      delete: (id) => req(`/instagram/contacts/${id}`, { method: 'DELETE' }),
    },
    templates: {
      list: () => req('/instagram/templates'),
      save: (data) => req('/instagram/templates', { method: 'POST', body: JSON.stringify(data) }),
      update: (id, data) => req(`/instagram/templates/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
      delete: (id) => req(`/instagram/templates/${id}`, { method: 'DELETE' }),
    },
    campaigns: {
      list: () => req('/instagram/campaigns'),
      create: (data) => req('/instagram/campaigns', { method: 'POST', body: JSON.stringify(data) }),
    },
    workflows: {
      list: () => req('/instagram/workflows'),
      save: (data) => req('/instagram/workflows', { method: 'POST', body: JSON.stringify(data) }),
      logs: (id) => req(`/instagram/workflows/${id}/logs`),
    },
    settings: {
      get: () => req('/instagram/settings'),
      save: (data) => req('/instagram/settings', { method: 'POST', body: JSON.stringify(data) }),
    },
    analytics: () => req('/instagram/analytics'),
  },
  retarget: {
    list: ({ search = '', status = '', category = '', sent = '', active = '', page = 1, limit = 20 } = {}) => {
      const qs = new URLSearchParams({
        search, status, category, sent, active,
        page: String(page), limit: String(limit),
      });
      return req(`/retarget/customers?${qs}`);
    },
    get: (id) => req(`/retarget/customers/${id}`),
    create: (data) => req('/retarget/customers', { method: 'POST', body: JSON.stringify(data) }),
    update: (id, data) => req(`/retarget/customers/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
    delete: (id) => req(`/retarget/customers/${id}`, { method: 'DELETE' }),
    // Bulk-import from a .csv/.xlsx file. Uses raw fetch + FormData so the
    // browser sets the multipart boundary (the shared req() helper forces JSON).
    importFile: (file) => {
      const form = new FormData();
      form.append('file', file);
      return fetch('/api/retarget/import', { method: 'POST', credentials: 'include', body: form })
        .then(async res => {
          if (!res.ok) {
            const text = await res.text().catch(() => '');
            let msg = text;
    ...
    sheet: {
      get: () => req('/retarget/sheet'),
      connect: (sheetUrl, sheetName) =>
        req('/retarget/sheet', { method: 'POST', body: JSON.stringify({ sheetUrl, sheetName }) }),
      syncNow: () => req('/retarget/sheet/sync-now', { method: 'POST' }),
    },
    syncHistory: (limit = 20) => req(`/retarget/sync-history?limit=${limit}`),
  },
};
