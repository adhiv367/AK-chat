...
// Design tokens pulled from the shared dark theme (constants.js -> C / index.css).
// `wa` / `waHeader` are intentionally kept as WhatsApp's own brand green —
// the phone-preview mock below deliberately mimics the real WhatsApp app
// chrome (which is not part of our dark theme) so merchants can see an
// accurate preview of what their template will look like on a phone.
const B = {
  bg: C.pageBg,
  card: C.cardBg,
  cardBorder: C.border,
  ...
  sectionBg: C.surfaceAlt,
  sectionBorder: C.borderDark,
  divider: C.borderDark,
  rowSep: C.borderDark,
  t1: C.text,
  t2: C.text,
  t3: C.textSecondary,
...
const CATEGORIES = [
  ...
  { id: 'UTILITY', icon: Bell, label: 'Utility', color: '#60A5FA', bg: 'rgba(96,165,250,0.16)', desc: 'Order updates, delivery alerts' },
...
const STATUSES = {
  DRAFT: { color: B.t6, bg: 'rgba(152,162,181,0.14)', dot: 'var(--c-textMuted)', label: 'DRAFT' },
  ...
  PAUSED: { color: 'var(--c-amber)', bg: 'rgba(251,191,36,0.16)', dot: 'var(--c-amber)', label: 'PAUSED' },
  DISABLED: { color: 'var(--c-textSecondary)', bg: 'rgba(152,162,181,0.14)', dot: 'var(--c-textSecondary)', label: 'DISABLED' },
};

const QUALITY_STYLES = {
  GREEN: { color: 'var(--c-green)', label: 'Quality: Green' },
  YELLOW: { color: 'var(--c-amber)', label: 'Quality: Yellow' },
  RED: { color: '#F87171', label: 'Quality: Red' },
};
  ...
  return (
    <div style={{ background: B.card, border: `1px solid ${B.cardBorder}`, borderRadius: 12, padding: '16px 18px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        <span style={{ width: 22, height: 22, borderRadius: 99, background: B.accent, color: C.primaryText, fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontFamily: FONT }}>{n}</span>
        <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--c-textMuted)', fontWeight: 700, fontFamily: FONT }}>{title}</span>
        {note && <span style={{ fontSize: 10, color: 'var(--c-textMuted)', fontFamily: FONT, textTransform: 'none', letterSpacing: 0, fontStyle: 'italic' }}>{note}</span>}
      </div>
      {children}
    </div>
  );
    ...
    <PhoneFrame headerTitle="Your Business" headerSubtitle="online" minHeight={330}>
      ...
      {!hasBubble && !hasButtons ? (
        ...
        <div style={{ marginLeft: 'auto', maxWidth: '88%', minWidth: '55%' }}>
            ...
            {headerType === 'DOCUMENT' && (
              headerMediaLibraryId
                ? <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 8px', background: 'rgba(0,0,0,.06)', borderRadius: 6, marginBottom: 7 }}>
                    <div style={{ width: 34, height: 38, background: '#FFFFFF', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 1px 3px rgba(0,0,0,.15)', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: '#111B21', fontFamily: FONT }}>Document</div>
                      <div style={{ fontSize: 9.5, color: '#667781', marginTop: 1, fontFamily: FONT }}>From Media Library</div>
                    </div>
                  </div>
                : <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 8px', background: 'rgba(0,0,0,.06)', borderRadius: 6, marginBottom: 7 }}><div style={{ width: 34, height: 38, background: '#FFFFFF', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 1px 3px rgba(0,0,0,.15)', position: 'relative', overflow: 'hidden' }}><div style={{ position: 'absolute', top: 0, right: 0, width: 10, height: 10, background: '#e94235', clipPath: 'polygon(100% 0,0 0,100% 100%)' }} /><FileText size={16} color="#9e9e9e" /></div><div><div style={{ fontSize: 11, fontWeight: 600, color: '#111B21', fontFamily: FONT }}>document.pdf</div><div style={{ fontSize: 9.5, color: '#667781', marginTop: 1, fontFamily: FONT }}>PDF · 1 page</div></div></div>
            )}
            {headerType === 'TEXT' && headerText && <div style={{ fontSize: 13, fontWeight: 700, color: '#111B21', marginBottom: 4, lineHeight: 1.4, fontFamily: FONT }}>{headerText}</div>}
            {bodyText && <div style={{ fontSize: 13.5, color: '#111B21', lineHeight: 1.55, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: FONT }}>{bodyText}</div>}
            {securityRec && <div style={{ fontSize: 11, color: '#667781', marginTop: 4, fontFamily: FONT }}>🔒 For your security, do not share this code.</div>}
            {codeExpiry && <div style={{ fontSize: 11, color: '#667781', marginTop: 2, fontFamily: FONT }}>⏱ This code expires in {codeExpiry} minute{+codeExpiry !== 1 ? 's' : ''}.</div>}
            {footerText && <div style={{ fontSize: 11.5, color: '#667781', marginTop: 4, fontFamily: FONT }}>{footerText}</div>}
            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 3, marginTop: 2, marginBottom: -1 }}>
              <span style={{ fontSize: 10.5, color: '#667781', fontFamily: FONT }}>9:41</span>
          ...
          {hasButtons && (
            <div style={{ background: '#FFFFFF', borderRadius: '0 0 7.5px 7.5px', overflow: 'hidden', boxShadow: '0 1px 0.5px rgba(11,20,26,.13)' }}>
              {buttons.map((btn, i) => (
...
function ButtonsSection({ buttons, onAdd, onRemove, onUpdate, category, errors }) {
  ...
  const tColor = { QUICK_REPLY: B.accent, URL: '#60A5FA', PHONE_NUMBER: B.green, OTP: B.green, COPY_CODE: '#8E24AA' };
  const tBg = { QUICK_REPLY: B.accentBg, URL: 'rgba(96,165,250,0.16)', PHONE_NUMBER: B.greenBg, OTP: B.greenBg };
  ...
  return (
    <Sec n={6} title="Buttons" note="optional">
        ...
        {buttons.length < 3 && !(isAuth && hasOtp) && (
          <div style={{ position: 'relative' }}>
            <button
              ...
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--c-border)'; e.currentTarget.style.color = 'var(--c-textSecondary)'; e.currentTarget.style.background = C.primaryText; }}
            >+ Add Button</button>
            {menuOpen && (
              <div style={{ position: 'absolute', right: 0, top: '110%', background: 'var(--c-cardBg)', border: '1px solid var(--c-border)', borderRadius: 10, padding: 6, boxShadow: '0 8px 32px rgba(0,0,0,.12)', zIndex: 50, minWidth: 200 }}>
                {menuOpts.map(o => (
                  <button key={o.type} disabled={o.disabled}
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '9px 12px', border: 'none', background: 'none', fontSize: 12, fontFamily: FONT, cursor: o.disabled ? 'not-allowed' : 'pointer', fontWeight: 600, borderRadius: 8, color: o.disabled ? 'var(--c-textMuted)' : B.t2, transition: 'background .1s' }}
                    onMouseEnter={e => { if (!o.disabled) e.currentTarget.style.background = B.bg; }}
                    onMouseLeave={e => e.currentTarget.style.background = 'none'}>
                    <span style={{ fontSize: 16 }}>{o.icon}</span>
                    <div><div>{o.label}</div><div style={{ fontSize: 10, color: o.disabled ? 'var(--c-border)' : B.t6, fontWeight: 500 }}>{o.disabled ? o.disabledMsg : o.hint}</div></div>
                  </button>
                ))}
              </div>
            )}
      ...
      {buttons.length === 0 && <div style={{ textAlign: 'center', padding: '14px 0', color: 'var(--c-textMuted)', fontSize: 12, fontFamily: FONT }}>No buttons added yet.</div>}
            ...
            <div key={i} style={{ border: `1.5px solid ${hasErr ? B.red + '44' : B.innerBorder}`, borderRadius: 10, padding: '12px 14px', background: B.innerBg }}>
              ...
              {btn.type === 'OTP' ? (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  ...
                  <div>
                    <Lbl>OTP Type</Lbl>
                    <select style={{ border: '1.5px solid var(--c-border)', borderRadius: 8, padding: '7px 12px', fontSize: 12, fontFamily: FONT, width: '100%', background: 'var(--c-cardBg)', color: 'var(--c-text)', outline: 'none', cursor: 'pointer', appearance: 'none' }} value={btn.otpType || 'COPY_CODE'} onChange={e => onUpdate(i, 'otpType', e.target.value)}>
...
function TemplateList({ templates, loading, onAdd, onEdit, onDelete, onView, onBulkDelete, onDuplicate, onBulkSubmit, onSyncAll, syncingAll, accounts, groupByTranslation, onToggleGroup }) {
  ...
  const chipStyle = (active) => ({
    ...
    color: active ? C.primaryText : B.t3, cursor: 'pointer',
  });
    ...
    <div style={{ padding: 24, fontFamily: FONT }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button
            onClick={onAdd}
            style={{ padding: '10px 18px', background: C.primary, color: C.primaryText, border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'flex', alignItems: 'center', gap: 6, transition: 'background .15s' }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        {selectedDrafts.length > 0 && (
          <button
            ...
            style={{ padding: '8px 14px', background: B.green, color: C.primaryText, border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: FONT, display: 'flex', alignItems: 'center', gap: 6 }}
          >
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 20px', background: B.card, border: `1px solid ${B.cardBorder}`, borderRadius: 12 }}>
          <LayoutTemplate size={40} color="var(--c-border)" style={{ marginBottom: 12 }} />
          ...
          <button
            onClick={onAdd}
            style={{ padding: '10px 18px', background: C.primary, color: C.primaryText, border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: 6 }}
          >
            <Plus size={16} /> Add Template
          </button>
        </div>
            ...
            <tbody>
              {filtered.map(t => {
                ...
                return (
                  <tr
                    key={t.id}
                    style={{ borderBottom: `1px solid ${B.rowSep}`, background: sel.isSelected(t.id) ? 'var(--c-primaryLight)' : 'transparent', transition: 'background .12s' }}
                    onMouseEnter={e => { if (!sel.isSelected(t.id)) e.currentTarget.style.background = 'var(--c-hover)'; }}
                    onMouseLeave={e => { if (!sel.isSelected(t.id)) e.currentTarget.style.background = 'transparent'; }}
                  >
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      {t.whatsappAccountName ? (
                        ...
                        <span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 99, background: 'rgba(251,191,36,0.16)', color: 'var(--c-amber)', fontWeight: 600, fontFamily: FONT }}>Unassigned</span>
                      )}
                    </td>
                    <td style={{ padding: '12px 14px' }}>
                          ...
                          {t._siblings.map(s => (
                            <button
                              ...
                              style={{
                                ...
                                border: `1px solid ${STATUSES[s.status]?.bg || 'var(--c-borderDark)'}`,
...
function BuilderView({ template, onBack, onSave, readOnly, accounts }) {
  ...
  const btnPriStyle = { padding: '10px 22px', background: B.accent, color: C.primaryText, border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: FONT, transition: 'all .15s', display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' };
  ...
  const hdrTabOnStyle = { ...hdrTabStyle, background: 'var(--c-text)', color: C.primaryText, borderColor: 'var(--c-text)' };
      ...
      <div>
        ...
        <div style={{ marginBottom: 20, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {!readOnly && (
              <button
                style={{ ...btnPriStyle, background: canSubmit ? B.green : 'var(--c-border)' }}
          ...
          <div onClick={() => setTranslationOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
            <div onClick={e => e.stopPropagation()} style={{ background: 'var(--c-cardBg)', borderRadius: 14, width: 440, padding: 22, boxShadow: C.shadowLg, fontFamily: FONT }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <select value={translationLang} onChange={e => setTranslationLang(e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--c-border)', borderRadius: 8, fontSize: 13, fontFamily: FONT, outline: 'none', background: 'var(--c-cardBg)', color: B.t2, boxSizing: 'border-box' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                ...
                <button onClick={handleAddTranslation} disabled={translationSaving} style={{ ...btnPriStyle, background: translationSaving ? 'var(--c-border)' : B.green }}>
          ...
          <div onClick={() => setTestOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
            <div onClick={e => e.stopPropagation()} style={{ background: 'var(--c-cardBg)', borderRadius: 14, width: 440, padding: 22, boxShadow: C.shadowLg, fontFamily: FONT }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                ...
                <button onClick={handleTestSend} disabled={testSending || !testTo.trim()} style={{ ...btnPriStyle, background: testSending || !testTo.trim() ? 'var(--c-border)' : B.green }}>
        ...
        {isApprovedEdit && !readOnly && (
          <div style={{ background: 'rgba(251,191,36,0.14)', border: '1px solid rgba(251,191,36,0.4)', borderRadius: 10, padding: '10px 14px', marginBottom: 14, fontSize: 12, color: 'var(--c-amber)', fontFamily: FONT }}>
        ...
        {status === 'PAUSED' && (
          <div style={{ background: 'rgba(251,191,36,0.16)', border: '1px solid rgba(251,191,36,0.4)', borderRadius: 10, padding: '12px 16px', marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--c-amber)', fontFamily: FONT }}>⏸ Template paused by Meta</div>
            <div style={{ fontSize: 11, color: 'var(--c-amber)', marginTop: 4, opacity: 0.85, fontFamily: FONT }}>
        ...
        {status === 'DISABLED' && (
          <div style={{ background: 'rgba(152,162,181,0.14)', border: '1px solid var(--c-border)', borderRadius: 10, padding: '12px 16px', marginBottom: 16, fontSize: 13, fontWeight: 700, color: 'var(--c-textSecondary)', fontFamily: FONT }}>
        ...
        {template?.previous_category && template.previous_category !== category && (
          <div style={{ background: 'rgba(251,191,36,0.16)', border: '1px solid rgba(251,191,36,0.4)', borderRadius: 10, padding: '10px 16px', marginBottom: 16, fontSize: 12, color: 'var(--c-amber)', fontFamily: FONT }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <Sec n={1} title="Basic Information">
              <div style={{ marginBottom: 12 }}>
                ...
                {accounts && accounts.length > 1 ? (
                  ...
                  <div style={{ padding: '8px 12px', background: 'rgba(251,191,36,0.16)', border: `1px solid var(--c-amber)`, borderRadius: 8, fontSize: 12, color: 'var(--c-amber)', fontFamily: FONT }}>
                ...
                {CATEGORIES.map(cat => {
                  ...
                  return (
                    <button key={cat.id} style={on ? catCardOnStyle : catCardStyle} onClick={() => !readOnly && handleCategoryChange(cat.id)} disabled={readOnly}>
                      ...
                      <div style={{ fontSize: 11, color: on ? 'var(--c-primaryText)' : B.t6, lineHeight: 1.45, fontFamily: FONT }}>{cat.desc}</div>
                    </button>
                  );
                })}
              </div>
                        ...
                        <div style={{ position: 'relative', width: '100%', height: 160 }}>
                          ...
                          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.25)' }}>
                            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(255,255,255,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <Play size={16} color="var(--c-text)" fill="var(--c-text)" />
                            </div>
                          </div>
                        </div>
                      ) : (
            ...
            <Sec n={4} title="Body (Message Content)">
              ...
              {bodyVars.length > 0 && (
                <div style={{ marginTop: 14, padding: '12px 14px', background: B.sectionBg, border: `1px solid ${showErrors && errors.bodySamples ? B.red + '55' : B.sectionBorder}`, borderRadius: 10 }}>
                  <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--c-textMuted)', fontWeight: 700, marginBottom: 6, fontFamily: FONT }}>Sample Values for Variables</div>
            ...
            {!readOnly && (
              <div style={{ background: B.card, border: `1px solid ${B.cardBorder}`, borderRadius: 12, padding: '16px 18px' }}>
                <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--c-textMuted)', fontWeight: 700, marginBottom: 14, fontFamily: FONT }}>Submit for Meta Approval</div>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button style={{ ...btnPriStyle, background: canSubmit ? B.green : 'var(--c-border)', flex: 1, justifyContent: 'center', fontSize: 14, padding: 12 }} onClick={handleSubmit} disabled={saving}>
            ...
            {showPayload && (
              <div style={{ background: B.card, border: `1px solid ${B.cardBorder}`, borderRadius: 12, padding: '16px 18px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--c-textMuted)', fontWeight: 700, fontFamily: FONT }}>API Payload</div>
                ...
                <div style={{ background: '#1A1A2E', borderRadius: 10, padding: '14px 16px', overflow: 'auto', fontFamily: "'DM Mono', monospace", fontSize: 11, color: 'var(--c-borderDark)', lineHeight: 1.7, maxHeight: 320 }}>
          ...
          <div style={{ position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--c-textMuted)', fontWeight: 700, fontFamily: FONT }}>Live Preview</span>
            ...
            <div style={{ background: B.card, border: `1px solid ${B.cardBorder}`, borderRadius: 12, padding: '14px 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--c-textMuted)', fontWeight: 700, fontFamily: FONT }}>Submission Checklist</div>
                {errCount === 0
            ...
            {/* Spec */}
            <div style={{ background: B.card, border: `1px solid ${B.cardBorder}`, borderRadius: 12, padding: '14px 16px' }}>
              <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--c-textMuted)', fontWeight: 700, marginBottom: 10, fontFamily: FONT }}>Template Spec</div>
              {[
                ['Name', name || '—', true],
    ...
    >
      ...
      >
        <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--c-border)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <Library size={20} color="var(--c-purple)" />
          <div style={{ flex: 1 }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 16, background: 'var(--c-surfaceAlt)' }}>
          {error && (
            <div style={{ padding: 10, background: 'var(--c-primaryLight)', color: '#F87171', borderRadius: 6, fontSize: 13, marginBottom: 12 }}>{error}</div>
          )}
          {!media ? (
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: `2px solid ${isSel ? 'var(--c-purple)' : 'var(--c-border)'}`,
                  ...
                  >
                    <div style={{
                      ...
                      background: targetType === 'image' ? 'rgba(59,130,246,0.14)' : targetType === 'video' ? 'rgba(139,92,246,0.14)' : 'rgba(245,158,11,0.14)',
                    }}>
                      {targetType === 'image' ? (
                        ...
                        <Icon size={40} color={targetType === 'video' ? 'var(--c-purple)' : 'var(--c-amber)'} />
                      )}
                      {isSel && (
                        <div style={{ position: 'absolute', top: 6, right: 6, background: 'var(--c-purple)', color: C.primaryText, borderRadius: '50%', width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--c-border)', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              background: !selected ? 'var(--c-hover)' : 'var(--c-purple)', color: C.primaryText,
