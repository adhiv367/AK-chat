...
import { Users, Search, Phone, User, Pencil, Trash2, Loader2, ChevronDown, X, Tag, Send, Play, Music, FileText, Image, Video, Library, Upload, FileSpreadsheet, CheckCircle2, AlertTriangle, Download, UserPlus, Mail, Link2 } from 'lucide-react';
...
// Retarget-aware "Source" badge. Contacts created/updated by the Retarget
// module are stamped with custom_fields.isRetarget + custom_fields.retargetType
// (contactSyncService.js's upsertContactFromRetarget) — everything else is
// a "Normal" contact.
const RETARGET_BADGE_STYLES = {
  Cart: { bg: 'var(--c-tintWarn)', color: 'var(--c-tintWarnText)', border: 'rgba(251,191,36,0.35)' },
  Checkout: { bg: 'var(--c-tintDanger)', color: 'var(--c-tintDangerText)', border: 'rgba(248,113,113,0.35)' },
  Product: { bg: 'var(--c-tintInfo)', color: 'var(--c-tintInfoText)', border: 'rgba(129,140,248,0.35)' },
  Collection: { bg: 'var(--c-tintSuccess)', color: 'var(--c-tintSuccessText)', border: 'rgba(52,211,153,0.35)' },
  Other: { bg: 'var(--c-tintNeutral)', color: 'var(--c-textSecondary)', border: 'var(--c-border)' },
};

function getRetargetInfo(contact) {
  const cf = contact?.custom_fields || {};
  if (!cf.isRetarget) return null;
  return {
    type: cf.retargetType || 'Other',
    url: cf.retargetURL || null,
    timestamp: cf.retargetTimestamp || null,
    lastSync: cf.retargetLastSync || null,
    source: cf.retargetSource || null,
  };
}

function SourceBadge({ contact }) {
  const retarget = getRetargetInfo(contact);
  if (!retarget) {
    return (
      <span style={{
        display: 'inline-flex', alignItems: 'center', gap: 4,
        padding: '3px 9px', borderRadius: 4,
        background: 'var(--c-tintNeutral)', color: C.textSecondary,
  ...
  const style = RETARGET_BADGE_STYLES[retarget.type] || RETARGET_BADGE_STYLES.Other;
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: 4,
      padding: '3px 9px', borderRadius: 4,
...
export default function ContactsPage({ user }) {
  ...
  const [sourceFilter, setSourceFilter] = useState('all'); // all | normal | retarget | Cart | Checkout | Product | Collection | Other
  ...
  // ── Import from Sheet (persistent auto-sync, repoints to Admin Settings'
  // Google Sheet engine — see openSheetImport/runSheetImport below) ────────
  const [sheetImportModal, setSheetImportModal] = useState(false);
  const [sheetImportUrl, setSheetImportUrl] = useState('');
  const [savedSheetUrl, setSavedSheetUrl] = useState(''); // URL currently persisted in google_sheet_settings, if any
  const [sheetSettingsLoading, setSheetSettingsLoading] = useState(false);
  const [sheetImporting, setSheetImporting] = useState(false);
  const [sheetImportError, setSheetImportError] = useState('');
  const [sheetImportResult, setSheetImportResult] = useState(null);
  ...
  // ── Add Contact (manual entry) ──────────────────────────────────────────
  // Restriction-free by design: phone can be typed in any common format
  // (spaces, dashes, +91, bare 10-digit) — the backend normalizes it via the
  // same normalizePhone() the Excel/Google Sheet imports use. api.saveContact
  // creates a new contact if the number doesn't exist yet, or updates it if
  // it does — same dedupe rule as every other entry point.
  const openAddContact = () => {
    setAddContactName(''); setAddContactPhone(''); setAddContactEmail('');
    setAddContactError(''); setAddContactModal(true);
  };
  ...
  const saveAddContact = async () => {
    if (!selectedNumber) return;
    if (!addContactName.trim()) { setAddContactError('Enter a name.'); return; }
    if (!addContactPhone.trim()) { setAddContactError('Enter a phone number.'); return; }
    setAddContactSaving(true); setAddContactError('');
    try {
      await api.saveContact(
        ...
        addContactName.trim(),
        [],
        addContactEmail.trim() ? { email: addContactEmail.trim() } : undefined,
      );
      setAddContactModal(false);
      loadContacts(true); // refresh the list — new/updated contact appears immediately
    } catch (err) {
      setAddContactError(err.message || 'Failed to save contact. Please try again.');
    } finally {
      setAddContactSaving(false);
    }
  };

  // ── Import from Sheet (persistent auto-sync) ─────────────────────────────
  // This button now repoints to the SAME engine as Admin Settings' Google
  // Sheet auto-sync (google_sheet_settings + sheetSyncScheduler): opening the
  // modal shows whatever sheet is currently configured; "Save & Sync" saves
  // the URL (which is what the background scheduler reads every tick) and
  // triggers one immediate sync so new contacts show up right away. There is
  // no separate one-time-only path anymore — new/changed rows in the sheet
  // keep creating/updating contacts automatically after this, with no more
  // manual imports needed.
  const openSheetImport = async () => {
    setSheetImportError(''); setSheetImportResult(null); setSheetImportModal(true);
    setSheetSettingsLoading(true);
    try {
      const settings = await api.googleSheetSettings.get();
      const url = settings?.sheetUrl || '';
      setSheetImportUrl(url);
      setSavedSheetUrl(url);
    } catch (err) {
      // Non-admin users get a 403 here (Google Sheet settings are admin-only)
      setSheetImportError(err.message || 'Could not load current sheet settings.');
      setSheetImportUrl(''); setSavedSheetUrl('');
    } finally {
      setSheetSettingsLoading(false);
    }
  };
  const closeSheetImport = () => {
    if (sheetImporting) return;
    setSheetImportModal(false);
  };
  const runSheetImport = async () => {
    const url = sheetImportUrl.trim();
    if (!url || !selectedNumber) return;
    setSheetImporting(true); setSheetImportError('');
    try {
      // Saving a URL that differs from what's currently configured resets the
      // sync cursor server-side, so a new sheet always starts reading from
      // its own row 1 rather than continuing whatever row count the old
      // sheet had reached.
      await api.googleSheetSettings.save(url);
      const res = await api.googleSheetSettings.syncNow();
      setSheetImportResult(res);
      setSavedSheetUrl(url);
      loadContacts(true); // refresh the list — new/updated contacts appear immediately
    } catch (err) {
      setSheetImportError(err.message || 'Sync failed. Please try again.');
    } finally {
      setSheetImporting(false);
    }
  };

  // Ctrl+V paste a sheet into the open modal (akchat convention: file inputs support paste)
  useEffect(() => {
  ...
  const filtered = contacts.filter(c => {
    ...
    const matchesSource = (() => {
      ...
      const retarget = getRetargetInfo(c);
      if (sourceFilter === 'normal') return !retarget;
      if (sourceFilter === 'retarget') return !!retarget;
      return retarget?.type === sourceFilter; // Cart | Checkout | Product | Collection | Other
    })();
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 16, flexWrap: 'wrap' }}>
          ...
          <select
            ...
            style={{
              ...
              color: C.text, fontSize: 13, fontWeight: 600, fontFamily: FONT,
              cursor: 'pointer',
            }}
          >
            <option value="all">All Contacts</option>
            <option value="normal">Normal Contacts</option>
            <option value="retarget">Retarget Contacts</option>
            <option value="Cart">Retarget — Cart</option>
            <option value="Checkout">Retarget — Checkout</option>
            <option value="Product">Retarget — Product</option>
            <option value="Collection">Retarget — Collection</option>
            <option value="Other">Retarget — Other</option>
          </select>
          ...
          {isAdmin && (
            <button
              ...
              onMouseLeave={e => { e.currentTarget.style.background = C.cardBg; }}
            >
              <Link2 size={15} /> Import from Sheet
            </button>
          )}
          ...
          {(search || filterTagIds.length > 0 || sourceFilter !== 'all') && (
            <button
              onClick={() => { setSearch(''); setFilterTagIds([]); setSourceFilter('all'); }}
              style={{
                display: 'flex', alignItems: 'center', gap: 4,
                padding: '8px 12px', borderRadius: 8,
                border: `1px solid ${C.border}`, background: 'var(--c-cardBg)',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: 'center', color: C.textMuted, fontSize: 13, padding: 60 }}>
            ...
            <div>{(search || filterTagIds.length > 0 || sourceFilter !== 'all') ? 'No contacts match your filters' : 'No saved contacts yet'}</div>
            {!search && (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: FONT, fontSize: 13 }}>
              <thead>
                <tr style={{ background: 'var(--c-hover)' }}>
                  ...
                  <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: C.textSecondary, borderBottom: `1px solid ${C.border}` }}>Source</th>
                  {categories.map(cat => (
              ...
              <tbody>
                {filtered.map(c => {
                  ...
                  // Contacts auto-created/updated by the Google Sheet sync are stamped
                  // with custom_fields.source === 'shopify_sheet_sync' (contactSyncService.js).
                  // Tint their row so it's visually obvious where they came from — no tag,
                  // just a background color, per how the rest of the table already works.
                  const isSheetSynced = c.custom_fields?.source === 'shopify_sheet_sync';
                  // Contacts mirrored from the Retarget module (custom_fields.isRetarget,
                  // written by contactSyncService.js's upsertContactFromRetarget). Retarget
                  // takes visual precedence over the plain sheet-sync tint — a left border
                  // plus a soft tint, so it stays visible across hover/selected states too.
                  const retarget = getRetargetInfo(c);
                  const retargetStyle = retarget ? (RETARGET_BADGE_STYLES[retarget.type] || RETARGET_BADGE_STYLES.Other) : null;
                  const baseBg = isSelected
                    ? 'var(--c-rowSelected)'
                    : retarget ? retargetStyle.bg : (isSheetSynced ? 'var(--c-tintInfo)' : 'var(--c-cardBg)');
                  const hoverBg = isSelected
                    ? 'var(--c-rowSelectedHover)'
                    : retarget ? retargetStyle.bg : (isSheetSynced ? 'var(--c-tintInfo)' : 'var(--c-rowHover)');
                  const leftBorder = retarget ? `3px solid ${retargetStyle.color}` : undefined;
                  return (
                    <tr key={c.contact_number} style={{
                      background: baseBg, borderBottom: `1px solid ${C.border}`, cursor: 'pointer',
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 16px' }}><SourceBadge contact={c} /></td>
                      {categories.map(cat => {
          ...
          >
            ...
            <div style={{ padding: '20px 22px' }}>
              <p style={{ fontSize: 13, color: C.textSecondary, margin: '0 0 18px', lineHeight: 1.5 }}>
                Add contacts to <strong style={{ color: C.text }}>{selectedNumberName}</strong>. Upload any sheet with
                {' '}<strong style={{ color: C.text }}>name</strong>, <strong style={{ color: C.text }}>phone</strong>, and optionally
                {' '}<strong style={{ color: C.text }}>email</strong> columns — column names like "Customer Name", "Mobile",
                "Contact Number" etc. are recognized automatically, and phone numbers can include spaces, dashes, or a
                {' '}<span style={{ fontFamily: "'DM Mono', monospace" }}>+91</span> prefix. Any other columns are ignored.
              </p>
              ...
              {importResult ? (
                ...
                <div>
                  ...
                  {importResult.skipped?.length > 0 && (
                    <div style={{ background: 'var(--c-tintWarn)', border: '1px solid rgba(251,191,36,0.35)', borderRadius: 10, padding: '10px 14px', maxHeight: 160, overflowY: 'auto' }}>
              ...
              {importError && (
                <div style={{ marginTop: 14, background: 'var(--c-tintDanger)', color: 'var(--c-tintDangerText)', borderRadius: 8, padding: '10px 14px', fontSize: 13 }}>
                  {importError}
                </div>
              )}
            </div>
      ...
      {/* Add Contact Modal — manual entry, restriction-free like adding a
          contact on a phone: name + phone (+ optional email). Same
          create-or-update backend as import/sync, so it behaves identically
          to imported contacts (no visual or functional difference). */}
      {addContactModal && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }}
        ...
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ background: C.cardBg, borderRadius: 14, boxShadow: C.shadowLg, width: '100%', maxWidth: 420, fontFamily: FONT }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <UserPlus size={18} color={C.primary} />
                <h2 style={{ fontSize: 17, fontWeight: 700, color: C.text, margin: 0 }}>Add Contact</h2>
              </div>
              <button onClick={closeAddContact} style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: C.textMuted, display: 'flex', padding: 4 }}>
                <X size={18} />
              </button>
            </div>

            <div style={{ padding: '20px 22px' }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: C.textMuted, marginBottom: 6, display: 'block' }}>Customer Name</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${C.border}`, borderRadius: 8, padding: '9px 12px', marginBottom: 14 }}>
                <User size={15} color={C.textMuted} />
                <input
                  value={addContactName}
                  onChange={e => setAddContactName(e.target.value)}
                  placeholder="e.g. Priya Sharma"
                  style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontSize: 14, fontFamily: FONT, color: C.text }}
                  autoFocus
                />
              </div>

              <label style={{ fontSize: 12, fontWeight: 600, color: C.textMuted, marginBottom: 6, display: 'block' }}>Phone Number</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${C.border}`, borderRadius: 8, padding: '9px 12px', marginBottom: 4 }}>
                ...
                <input
                  ...
                  style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontSize: 14, fontFamily: FONT, color: C.text }}
                  onKeyDown={e => { if (e.key === 'Enter') saveAddContact(); }}
                />
              </div>
              <div style={{ fontSize: 11.5, color: C.textMuted, marginBottom: 14 }}>
                Any format works — spaces, dashes, or a +91 prefix are all fine.
              </div>

              <label style={{ fontSize: 12, fontWeight: 600, color: C.textMuted, marginBottom: 6, display: 'block' }}>Email (optional)</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${C.border}`, borderRadius: 8, padding: '9px 12px', marginBottom: 16 }}>
              ...
              {addContactError && (
                <div style={{ marginBottom: 14, background: 'var(--c-tintDanger)', color: 'var(--c-tintDangerText)', borderRadius: 8, padding: '10px 14px', fontSize: 13 }}>
                  {addContactError}
                </div>
              )}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                <button
                  ...
                  style={{
                    padding: '9px 16px', borderRadius: 8, border: `1px solid ${C.border}`, background: 'transparent',
                    color: C.text, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: FONT,
                  }}
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={addContactSaving || !addContactName.trim() || !addContactPhone.trim()}
                  style={{
                    display: 'inline-flex', alignItems: 'center', gap: 6,
                    padding: '9px 20px', borderRadius: 8, border: 'none',
                    background: (addContactSaving || !addContactName.trim() || !addContactPhone.trim()) ? '#ccc' : C.primary,
                    color: '#fff',
                    cursor: (addContactSaving || !addContactName.trim() || !addContactPhone.trim()) ? 'not-allowed' : 'pointer',
                    fontSize: 13, fontWeight: 700, fontFamily: FONT,
                  }}
                >
                  {addContactSaving ? <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Saving…</> : <>Save Contact</>}
                </button>
              </div>
            </div>
          </div>
      ...
      {/* Import from Sheet Modal — now the SAME persistent auto-sync engine
          as Admin Settings' Google Sheet section. Opening it shows whatever
          sheet is currently configured (blank if none yet); saving a
          different URL restarts the sync from that sheet's row 1. After
          "Save & Sync", the background scheduler keeps picking up new rows
          automatically — no more re-opening this modal to get new contacts. */}
      {sheetImportModal && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }}
          onClick={closeSheetImport}
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ background: C.cardBg, borderRadius: 14, boxShadow: C.shadowLg, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', fontFamily: FONT }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <Link2 size={18} color={C.primary} />
                <h2 style={{ fontSize: 17, fontWeight: 700, color: C.text, margin: 0 }}>Import from Sheet</h2>
              </div>
              <button onClick={closeSheetImport} style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: C.textMuted, display: 'flex', padding: 4 }}>
                <X size={18} />
              </button>
            </div>
            ...
            <div style={{ padding: '20px 22px' }}>
              {sheetImportResult ? (
                /* ---- Result summary — same layout as the Excel import result ---- */
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                    <CheckCircle2 size={20} color={C.green} />
                    <span style={{ fontSize: 15, fontWeight: 700, color: C.text }}>Import complete</span>
                  </div>
                  <div style={{ display: 'flex', gap: 10, marginBottom: sheetImportResult.skipped?.length ? 16 : 0 }}>
                    {[
                      { label: 'Added', value: sheetImportResult.imported, color: C.green },
                      { label: 'Updated', value: sheetImportResult.updated, color: C.purple },
                      { label: 'Skipped', value: sheetImportResult.skipped?.length || 0, color: sheetImportResult.skipped?.length ? C.primary : C.textMuted },
                    ].map(s => (
                      <div key={s.label} style={{ flex: 1, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px', textAlign: 'center' }}>
                        <div style={{ fontSize: 24, fontWeight: 700, color: s.color, fontFamily: "'DM Mono', monospace" }}>{s.value}</div>
                        <div style={{ fontSize: 11, color: C.textMuted, textTransform: 'uppercase', letterSpacing: '0.04em', marginTop: 2 }}>{s.label}</div>
                  ...
                  {sheetImportResult.skipped?.length > 0 && (
                    <div style={{ background: 'var(--c-tintWarn)', border: '1px solid rgba(251,191,36,0.35)', borderRadius: 10, padding: '10px 14px', maxHeight: 160, overflowY: 'auto' }}>
                      ...
                      {sheetImportResult.skipped.map((s, i) => (
                        <div key={i} style={{ fontSize: 12, color: '#7A6200', padding: '2px 0' }}>
                          Row {s.row}: {s.reason}
                        </div>
                      ))}
                ...
                <>
                  <p style={{ fontSize: 13, color: C.textSecondary, margin: '0 0 16px', lineHeight: 1.5 }}>
                    Add contacts to <strong style={{ color: C.text }}>{selectedNumberName}</strong> from any Google Sheet.
                    Name, phone, and email columns are detected automatically — no exact column names needed.
                    The sheet must be shared as <strong style={{ color: C.text }}>"Anyone with the link can view"</strong>.
                  </p>
                  <label style={{ fontSize: 12, fontWeight: 600, color: C.textMuted, marginBottom: 6, display: 'block' }}>Google Sheet URL</label>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${C.border}`, borderRadius: 8, padding: '9px 12px' }}>
                    <Link2 size={15} color={C.textMuted} />
                    <input
                      value={sheetImportUrl}
                      onChange={e => setSheetImportUrl(e.target.value)}
                      placeholder="https://docs.google.com/spreadsheets/d/.../edit"
                      disabled={sheetSettingsLoading}
                      style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontSize: 14, fontFamily: FONT, color: C.text }}
                      onKeyDown={e => { if (e.key === 'Enter') runSheetImport(); }}
                      autoFocus
                    />
                    {sheetSettingsLoading && <Loader2 size={14} color={C.textMuted} style={{ animation: 'spin 1s linear infinite' }} />}
                  </div>
                  <div style={{ fontSize: 11.5, color: C.textMuted, marginTop: 8 }}>
                    {savedSheetUrl
                      ? <>This is the sheet currently syncing to {selectedNumberName}. Clear it and paste a different URL to switch — the new sheet starts syncing from its first row.</>
                      : <>No sheet is connected to {selectedNumberName} yet. Saving one here starts continuous auto-sync: new rows in the sheet keep creating or updating contacts automatically, no manual re-import needed.</>}
                  </div>
                </>
              )}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
                ...
                >
                  {sheetImportResult ? 'Close' : 'Cancel'}
                </button>
                {!sheetImportResult && (
                  <button
                    onClick={runSheetImport}
                    disabled={!sheetImportUrl.trim() || sheetImporting || sheetSettingsLoading}
                    style={{
                      display: 'inline-flex', alignItems: 'center', gap: 6,
                      padding: '9px 20px', borderRadius: 8, border: 'none',
                      background: (!sheetImportUrl.trim() || sheetImporting || sheetSettingsLoading) ? '#ccc' : C.primary, color: '#fff',
                      cursor: (!sheetImportUrl.trim() || sheetImporting || sheetSettingsLoading) ? 'not-allowed' : 'pointer',
                      fontSize: 13, fontWeight: 700, fontFamily: FONT,
                    }}
                  >
                    {sheetImporting ? <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Saving & syncing…</> : <>Save & Sync</>}
                  </button>
                )}
              </div>
            </div>
      ...
      {detailContact && (
        <div style={{
          ...
          <div style={{
            ...
            {/* Retarget Information — read-only, populated by the Retarget
                module's Contacts bridge (contactSyncService.js). Only shown
                for contacts that actually came from Retarget. */}
            {(() => {
              const retarget = getRetargetInfo(detailContact);
              ...
              return (
                <div style={{
                  ...
                  <div style={{ fontSize: 12, fontWeight: 700, color: style.color, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    Retarget Information
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr', rowGap: 8, columnGap: 8, fontSize: 13 }}>
                    <div style={{ color: C.textMuted, fontWeight: 600 }}>Retarget Type</div>
                    <div style={{ color: C.text }}>{retarget.type}</div>

                    <div style={{ color: C.textMuted, fontWeight: 600 }}>Exit URL</div>
                    <div style={{ color: C.text, wordBreak: 'break-all' }}>
                      {retarget.url
                        ? <a href={retarget.url} target="_blank" rel="noreferrer" style={{ color: 'inherit' }}>{retarget.url}</a>
                        : '—'}
                    </div>

                    <div style={{ color: C.textMuted, fontWeight: 600 }}>Last Website Visit</div>
                    <div style={{ color: C.text }}>{fmt(retarget.timestamp)}</div>

                    <div style={{ color: C.textMuted, fontWeight: 600 }}>Last Sync</div>
                    <div style={{ color: C.text }}>{fmt(retarget.lastSync)}</div>

                    <div style={{ color: C.textMuted, fontWeight: 600 }}>Source</div>
                    <div style={{ color: C.text }}>{retarget.source || '—'}</div>
                  </div>
                </div>
              );
            })()}
