...
// ─── Design Tokens ────────────────────────────────────────────────────────────
// Design tokens are pulled from the shared dark theme (constants.js -> C),
// which maps to the CSS variables defined in index.css. Do NOT hardcode
// light-theme colors here — reuse the same palette as the rest of the app.
const B = {
  bg: C.pageBg,
  card: C.cardBg,
  cardBorder: C.border,
...
const STATUS_STYLES = {
  ...
  inactive: { color: B.t5, bg: 'rgba(152,162,181,0.14)', dot: B.t6, label: 'Inactive' },
...
function NewAutomationModal({ open, onClose, onCreate }) {
  ...
  const overlayStyle = { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' };
  const modalStyle = { background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 14, padding: '24px 28px', width: 440, maxWidth: '90vw', boxShadow: C.shadowLg };
  const inpStyle = { border: `1.5px solid ${C.border}`, borderRadius: 10, padding: '9px 14px', fontSize: 13, fontFamily: FONT, width: '100%', background: C.surfaceAlt, color: C.text, outline: 'none' };
  const btnPriStyle = { padding: '10px 22px', background: B.accent, color: C.primaryText, border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: FONT };
  const btnGhostStyle = { padding: '10px 22px', background: C.surfaceAlt, border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: FONT, color: C.textSecondary };
    ...
    <div style={{ padding: 24, fontFamily: FONT }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 1 360px', minWidth: 220 }}>
          ...
          <input
            style={{ flex: 1, border: `1.5px solid ${C.border}`, borderRadius: 8, padding: '8px 12px', fontSize: 13, fontFamily: FONT, background: C.surfaceAlt, color: C.text, outline: 'none' }}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 20px', background: B.card, border: `1px solid ${B.cardBorder}`, borderRadius: 12 }}>
          <Bot size={40} color={C.textMuted} style={{ marginBottom: 12 }} />
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: FONT }}>
            ...
            <tbody>
              {filtered.map(c => (
                <tr
                  key={c.id}
                  style={{ borderBottom: `1px solid ${B.rowSep}`, background: sel.isSelected(c.id) ? C.primaryLight : 'transparent', transition: 'background .12s' }}
                  onMouseEnter={e => { if (!sel.isSelected(c.id)) e.currentTarget.style.background = C.hover; }}
                  onMouseLeave={e => { if (!sel.isSelected(c.id)) e.currentTarget.style.background = 'transparent'; }}
                >
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={() => onEdit(c)} title="Edit" style={{ width: 28, height: 28, borderRadius: 6, border: `1.5px solid ${C.border}`, background: C.surfaceAlt, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: B.t4 }}>
                        <Pencil size={13} />
                      </button>
                      <button onClick={() => onDuplicate(c)} title="Duplicate (creates a disabled copy)" style={{ width: 28, height: 28, borderRadius: 6, border: `1.5px solid ${C.border}`, background: C.surfaceAlt, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: B.t4 }}>
                        <Copy size={13} />
                      </button>
                      <button onClick={() => setDeleteModal({ open: true, chatbot: c })} title="Delete" style={{ width: 28, height: 28, borderRadius: 6, border: `1.5px solid ${C.border}`, background: C.surfaceAlt, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: B.red }}>
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </td>
