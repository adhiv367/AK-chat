<#
  AKChat one-command installer — Windows (local / testing).
  ...
  Run in PowerShell, from inside the akchat folder:
      .\install.ps1
...
Say "akchat installer (Windows - local mode, http://localhost)"

# ── 1. Inputs ───────────────────────────────────────────────────────────────
$ADMIN_EMAIL = Read-Host "Admin email [admin@akchat.local]"
if (-not $ADMIN_EMAIL) { $ADMIN_EMAIL = 'admin@akchat.local' }
...
# mapping on the akchat-frontend service so http://localhost works out of the
# box. Idempotent: only inject if akchat-frontend doesn't already have that
...
foreach ($line in $composeLines) {
  if ($line -match '^  akchat-frontend:\s*$') { $inBlock = $true; continue }
...
if (-not $hasPort) {
  $patched = foreach ($line in $composeLines) {
    $line
    if ($line -match '^  akchat-frontend:\s*$') {
      '    ports:'
      '      - "80:80"'
    }
  }
...
DATABASE_URL=postgresql://postgres:$PGPASS@akchat-db:5432/postgres
POSTGRES_SSL=false
REDIS_URL=redis://redis:6379
JWT_SECRET=$JWT
AKCHAT_ENCRYPTION_KEY=$ENCKEY
...
docker compose up -d akchat-db redis
Write-Host -NoNewline "Waiting for the database to be ready"
do {
  Start-Sleep -Seconds 2
  Write-Host -NoNewline "."
  $health = (docker inspect -f '{{.State.Health.Status}}' akchat-db 2>$null)
...
"@ | docker exec -i akchat-db psql -U postgres -d postgres | Out-Null
Get-ChildItem "db\migrations\*.sql" | Sort-Object Name | ForEach-Object {
  Get-Content $_.FullName | docker exec -i akchat-db psql -U postgres -d postgres -v ON_ERROR_STOP=1 | Out-Null
}
OK "Database ready."

# ── 5. Start the app ────────────────────────────────────────────────────────
docker compose up -d akchat-backend akchat-frontend
