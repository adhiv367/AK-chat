...
async function syncAccountTemplates(account) {
  ...
  // Anything locally APPROVED that Meta no longer returned under the same
  // name+language has drifted (edited/removed in Business Manager, etc).
  // Flag it instead of leaving a stale APPROVED status that silently 404s at send time.
  const remoteKeys = new Set(remote.map(r => `${String(r.name).toLowerCase()}::${r.language}`));
  await pool.query(
    `UPDATE coexistence.message_templates
        SET status = 'DISABLED', rejection_reason = 'Not found on Meta at last sync — resubmit or duplicate'
      WHERE whatsapp_account_id = $1
        AND status = 'APPROVED'
        AND NOT (lower(name) || '::' || language) = ANY($2::text[])`,
    [account.id, [...remoteKeys]]
  );
