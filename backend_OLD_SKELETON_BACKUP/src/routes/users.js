...
router.get('/users', adminOnly, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT * FROM coexistence.akchat_users ORDER BY created_at`
    );
...
router.get('/users/:id', adminOnly, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT * FROM coexistence.akchat_users WHERE id = $1`,
      [req.params.id]
    );
...
router.patch('/users/:id', adminOnly, async (req, res) => {
  ...
  try {
    const { rows: existing } = await pool.query(`SELECT * FROM coexistence.akchat_users WHERE id = $1`, [id]);
...
router.post('/users/:id/reset-password', adminOnly, async (req, res) => {
  ...
  try {
    const { rows: existing } = await pool.query(
      `SELECT id, username FROM coexistence.akchat_users WHERE id = $1`,
      [id]
    );
    if (existing.length === 0) return res.status(404).json({ error: 'Not found' });
...
router.delete('/users/:id', adminOnly, async (req, res) => {
  ...
  try {
    ...
    const { rows: existing } = await pool.query(
      `SELECT username, role FROM coexistence.akchat_users WHERE id = $1`,
      [id]
    );
    if (existing.length === 0) return res.status(404).json({ error: 'Not found' });
    await pool.query(`DELETE FROM coexistence.akchat_users WHERE id = $1`, [id]);
    await auditLog({
