...
// AKCHAT_ENCRYPTION_KEY (falls back to JWT_SECRET so dev doesn't break).

const crypto = require('crypto');

const RAW = process.env.AKCHAT_ENCRYPTION_KEY || process.env.JWT_SECRET || '';
...
if (process.env.NODE_ENV === 'production' &&
    (!process.env.AKCHAT_ENCRYPTION_KEY ||
     WEAK_KEYS.has(process.env.AKCHAT_ENCRYPTION_KEY) ||
     process.env.AKCHAT_ENCRYPTION_KEY.length < 32)) {
  console.error('[crypto] FATAL: AKCHAT_ENCRYPTION_KEY must be a strong, unique value (>=32 chars) in production. Generate one with: openssl rand -hex 32');
  process.exit(1);
}
if (!RAW) {
  console.warn('[crypto] WARNING: neither AKCHAT_ENCRYPTION_KEY nor JWT_SECRET set — encryption will use empty key');
}
