...
/**
 * Turns a thrown error from the Meta send functions (metaSend.js) into a
 * readable string for storing in broadcast_logs.error_message. Prefers
 * Meta's own structured error detail when present (err.metaError), falling
 * back to the plain error message otherwise.
 */
function formatSendError(err) {
  if (err?.metaError) {
    const code = err.metaError.code ? ` (code ${err.metaError.code})` : '';
    const details = err.metaError.error_data?.details || '';
    return `${err.metaError.message || 'Meta error'}${code}${details ? ' — ' + details : ''}`;
  }
  return err?.message || 'Unknown send error';
}
