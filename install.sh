#!/usr/bin/env bash
#
# AKCHAT one-command installer — macOS & Linux.
#
...
if [ "$MODE" = server ]; then
  ...
  CORS="http://localhost"
  default_email="admin@akchat.local"
...
if [ "$MODE" = server ]; then
  ...
  sed -i.bak "s/akchatchat\.example\.com/$DOMAIN/g" Caddyfile && rm -f Caddyfile.bak
  ...
  # mapping on the akchat-frontend service so http://localhost works out of
  # the box. Idempotent: only inject if akchat-frontend doesn't already have
...
until [ "$(docker inspect -f '{{.State.Health.Status}}' akchat-db 2>/dev/null || true)" = "healthy" ]; do
  printf '.'; sleep 2
done
printf '\n'

say "Creating the schema and applying all migrations…"
docker compose exec -T akchat-db psql -U postgres -d postgres >/dev/null <<'SQL'
...
for f in $(ls db/migrations/*.sql | sort); do
  docker compose exec -T akchat-db psql -U postgres -d postgres -v ON_ERROR_STOP=1 < "$f" >/dev/null
done
ok "Database ready."

# ── 6. Start the app ────────────────────────────────────────────────────────
if [ "$MODE" = server ]; then
  docker compose up -d akchat-backend akchat-frontend caddy
  URL="https://$DOMAIN"
else
