...
async function seed() {
  ...
  },
  {
    ...
    const tpls = [
      [101, 'welcome_message', 'UTILITY', 'en', 'NONE', null, 'Hi {{1}}, welcome to AKchat! How can we help you today?', 'Reply STOP to opt out', '[{"type":"QUICK_REPLY","text":"Pricing"},{"type":"QUICK_REPLY","text":"Site Visit"}]', '{"1":"Customer"}', 'APPROVED'],
      [102, 'property_alert', 'MARKETING', 'en', 'TEXT', 'New Property Alert', 'Hi {{1}}, we found a {{2}} in {{3}} within your budget of {{4}}. Interested?', null, '[{"type":"QUICK_REPLY","text":"Yes"},{"type":"QUICK_REPLY","text":"No"}]', '{"1":"Anjali","2":"Product","3":"Location","4":"₹₹10,000"}', 'APPROVED'],
      [103, 'otp_verify', 'AUTHENTICATION', 'en', 'NONE', null, 'Your AKchat verification code is {{1}}. Valid for 10 minutes.', null, '[{"type":"OTP","text":"Copy Code","otpType":"COPY_CODE"}]', '{"1":"123456"}', 'APPROVED'],
      ...
      [105, 'brochure_send', 'UTILITY', 'en', 'NONE', null, 'Here is the brochure for {{1}}. Let us know if you have any questions!', null, '[{"type":"URL","text":"View Brochure","value":"https://example.com/brochure.pdf"}]', '{"1":"Location Product"}', 'APPROVED'],
    ];
    for (const t of tpls) {
      await client.query(`
    ...
    await client.query(`DELETE FROM coexistence.chatbots WHERE name IN ('Welcome Automation', 'Pricing Automation')`);
