...
const { isAutomationBlocked, SKIP_REASON } = require('../services/automationGuard');
...
// Normalize an AKchat field name into a {{variable}} token key.
    ...
    try {
      if (a.kind === 'Assign to BDA') {
        ...
        const { rows: uRows } = await client.query(
          `SELECT id, display_name, role, is_active FROM coexistence.akchat_users WHERE id = $1`,
          [userId]
        );
        if (uRows.length === 0) {
...
// Records that an automation was blocked by the Automation Guard so it
// shows up in Execution History as 'skipped' with a reason, instead of
// being silently ignored (per business requirement — easier debugging).
async function logSkippedExecution(client, automation, context, reason) {
  try {
    const { rows } = await client.query(
    ...
    console.error(`[engine] Failed to log skipped execution for automation ${automation.id}:`, err.message);
    return null;
  }
}
...
async function resumeAutomation(client, executionId, record) {
  ...
  // ── Automation Guard: Manual Reply tag blocks resuming paused workflows ──
  // A workflow that's mid-conversation (waiting for reply) must NOT continue
  // once a human agent has taken over via the Manual Reply tag.
  if (isAutomationBlocked(context.contact?.tags)) {
    console.log(`[engine] Skipped resume -- "Manual Reply" tag set for ${record.contact_number} (execution=${executionId})`);
    await client.query(
      `UPDATE coexistence.automation_executions
          SET status='skipped', awaiting_node_id=NULL, error_message=$2, completed_at=NOW()
        WHERE id=$1`,
      [executionId, SKIP_REASON]
    );
    return execRow;
  }
  ...
  try {
    ...
    for (const automation of automations) {
      ...
      if (matchesKeyword(messageRecord.message_body, keyword, matchType, caseSensitive)) {
        // ── Automation Guard: Manual Reply tag blocks ALL inbound automation ──
        if (isAutomationBlocked(context.contact?.tags)) {
          console.log(`[engine] Skipped -- "Manual Reply" tag set for ${messageRecord.contact_number} (automation=${automation.id})`);
          const skipped = await logSkippedExecution(client, automation, context, SKIP_REASON);
          if (skipped) executions.push(skipped);
          continue;
        }
