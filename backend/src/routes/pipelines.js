...
router.get('/numbers', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      ...
      FROM (
        -- Numbers with actual chat activity (original source).
        SELECT
          wa_number,
        ...
        -- Numbers that have Contacts (e.g. from Retarget CSV/Excel/Sheet import
        -- or manual add) but zero chat_history rows yet. Without this branch a
        -- freshly-imported wa_number never appears in the number picker, so
        -- ContactsPage.jsx never selects it and its contacts stay invisible
        -- even though they exist in coexistence.contacts. last_message_time is
        -- NULL here so numbers with real chat activity still sort first.
        SELECT
          wa_number,
        ...
        WHERE NOT EXISTS (
          SELECT 1 FROM coexistence.chat_history ch
           WHERE ch.wa_number = c.wa_number
             AND ch.timestamp >= NOW() - ${DEFAULT_DATA_WINDOW}
        )
        ${!isAdmin(req.user) ? `AND c.assigned_user_id = $${params.length}` : ''}
        GROUP BY wa_number
      ) combined
      GROUP BY wa_number
